...
<Image
  src="https://raw.githubusercontent.com/josepot/react-rxjs-slides/master/images/old-score.png"
  width="432px"
  height="265px"
/>
...
<Image
  src="https://raw.githubusercontent.com/josepot/react-rxjs-slides/master/images/new-score.png"
  width="432px"
  height="265px"
/>
...
<Notes>
- I will be presenting React-RxJS, which is a library that provides
a set of bindings so that we can use RxJS as a state-management library with React.

- Something to keep in mind, though, is that I work at Adaptive Financial Consulting,
where we specialice in building hihgly performant real-time data solutions. So,
please keep in mind that this library has been created to address some of the
challenges that we face with the kind of applications that we build, which are 
usually quite different from most web-applications out there.

- So, I'm not here to claim that this is the ultimate state-management solution.
There are many other amazing state-management libraries out there, that could
be a better fit for your needs.

- However, I think that there could be other people out there facing the same kind of
challenges that we are facing, and perhaps for those React-RxJS could be an interesting
alternative.
...
So, for the rest of the presentation I'm going to be using the code of
Reactive-Trader to explain things like: how thanks to react-rxjs we can leaverage
some features from React Concurrent Mode, how these features have helped us to drastically
improve the loading times, without compromising the way that we write our code,
and how thansk to react-rxjs we now have a code-base that's a lot easier to
"code-navigate".

Let's have a quick look at Reactive-Trader, we won't be doing a deep-dive on
all its features, but let me highlight its 3 different main sections and what they
are about:
- The live-rates: they allow the user to trade on a different set of assets
at the live rate that's displayed in the buy/sell buttons. If the nominal value
is too high, then they can not trade at the standard-market price, they need to request
for a quote... This assets can be filtered, and the user can change the view, in case that
they want to show/hide the historical prices of that asset.

- Then we have the Trades section, which is a table for displaying the latest
transactions, this ones can be filtered, sorted, downloaded into an CSV file, etc

- And the last section is the analytics, which has 3 different sub-sections:
- The global profit and loss chart, which at the top it displays the current
balance.

- Then a buble-chart that displays the volume for each currency

- And another the PnL bars, which display the balance per each currency-pair

Ok, now that we are a bit more familiar with what Reactive-Trader does,
...
## State Management vs Side-Effect Orchestration:
...
Nicholas Jamieson and Ben Lesh
