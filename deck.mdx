...
Why did we chose React-RxJS for the latest refactor?

Notice that RT is consuming a websocket API that mostly only sends deltas and
we need to combine that information with the user interactions, meaning that
we have to keep track of a lot of derived state... and as we saw before
React-RxJS is a really good tool for dealing with derived state, also our API
endpoints are observables, so it makes sense for us to keep deriving those
observables to manage the state of the application.
